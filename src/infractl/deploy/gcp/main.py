...
@click.argument('gpu_enabled', type=bool)
@click.argument('gpu_model')
def validate_gpu_settings(gpu_enabled, gpu_model):
    supported_gpus_dict = {
        "a2-highgpu":["nvidia-tesla-a100"],
        "a2-ultragpu": ["nvidia-a100-80gb"],
        "a3-highgpu": ["nvidia-h100-80gb"],
        "g2-standard": ["nvidia-l4"],
        "n1-standard": ["nvidia-tesla-t4", "nvidia-tesla-p4", "nvidia-tesla-v100", "nvidia-tesla-p100", "nvidia-tesla-k80"],
        "n1-highmem": ["nvidia-tesla-t4", "nvidia-tesla-p4", "nvidia-tesla-v100", "nvidia-tesla-p100", "nvidia-tesla-k80"],
        "n1-highcpu": ["nvidia-tesla-t4", "nvidia-tesla-p4", "nvidia-tesla-v100", "nvidia-tesla-p100", "nvidia-tesla-k80"],
    }
    ...
    if machine_type:
        command = f"gcloud compute machine-types list --filter=\"name='{machine_type}'\" --format='value(name)' --zones='{zone}' --verbosity=error"
        try:
            # Execute the command and check if the output contains the machine type
            result = run(command, shell=True, check=True, text=True, capture_output=True)
        ...
        command = f"gcloud compute accelerator-types list --verbosity=error | grep -i {zone} | grep -i {gpu_model}"
        try:
            # Execute the command and check if the output contains the machine type
            result = run(command, shell=True, check=True, text=True, capture_output=True)
    ...
    if simplified_machine_type in supported_gpus_dict:
        ...
        print(f"Error: Machine type \"{machine_type}\" (simplified to {simplified_machine_type}) is not recognized.")
        sys.exit(300)
